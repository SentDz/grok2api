...
HOOK_DEBUG = Path(
    os.environ.get(
        "STATSIG_HOOK_DEBUG",
        str(ROOT.parent.parent / "backend/internal/infra/provider/web/testdata/statsig_live_pair.debug.json"),
    )
)
...
class RepairPipelineTest(unittest.TestCase):
    def test_capture_pair_has_same_page_seed_hex_and_paths(self) -> None:
    ...
    def test_hook_capture_debug_has_seek_and_animation(self) -> None:
        if not HOOK_DEBUG.is_file():
            self.skipTest("Optional private capture fixture missing; set STATSIG_HOOK_DEBUG to supply it")
