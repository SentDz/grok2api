...
function computeHex(seed, paths) {
  const pathIndex = seed[5] % 4;
  const path = paths[pathIndex];
  const segments = pathSegments(path);
  const segIdx = seed[39] % 16;
  const seek = Math.round(((seed[3] % 16) * (seed[31] % 16) * (seed[36] % 16)) / 10) * 10;
  return hexFromSegment(segments[segIdx], seek, 4096);
}
