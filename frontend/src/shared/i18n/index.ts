    ...
    translation: {
      ...
      videoTask: {
        ...
        stages: {
          ...
          acquire_submit_egress: "准备视频提交出口与 Clearance", submit_egress_ready: "视频提交出口已就绪",
          video_node_rate_limited: "视频提交 429，节点冷却 3 分钟，账号额度保留", select_video_retry_node: "选择其他视频提交代理节点",
          video_node_cooldown_failed: "视频提交 429，节点冷却保存失败，停止重试", video_direct_rate_limited: "直连视频提交 429，账号额度保留",
          video_retry_egress_unavailable: "无其他可用提交节点，停止重试", video_rate_limit_retry_limit: "已尝试 5 个节点，停止 429 重试",
          statsig_prepare: "准备 Statsig 签名", statsig_cache: "命中 Statsig 签名缓存", statsig_wait: "获取签名或等待正在进行的签名请求",
          statsig_meta: "获取 Statsig 页面信息", statsig_meta_index: "抓取 Grok /index 签名页面", statsig_meta_root: "回退抓取 Grok 首页",
          statsig_sign_url: "调用 Statsig URL 签名服务", statsig_ready: "Statsig 签名准备完成", statsig_retry: "重新获取 Statsig 签名",
          statsig_stale: "签名刷新失败，使用已有签名", statsig_local_fallback: "回退使用本地 Statsig 签名", statsig_remote_failed: "远程 Statsig 签名失败",
          http_connect: "获取连接 / 建立代理隧道与 TLS", http_send: "连接已就绪，发送请求", http_wait_headers: "请求已发送，等待响应头",
          http_first_byte: "已收到响应首字节，读取响应头", http_headers: "已收到 HTTP 响应头", http_read_body: "读取 HTTP 响应内容",
      ...
      accounts: {
        ...
        webQuotaImage: "图片生成总余额",
        webQuotaImageEdit: "图片编辑总余额",
        webQuotaVideo: "视频生成总余额",
        webQuotaVideo720p: "720p 视频总余额",
        webQuotaSummary: "已同步 {{synced}} / {{total}} 个账号 · 已耗尽 {{exhausted}} 个",
        lastUsed: "最近使用",
        createdAt: "创建时间",
        name: "名称",
        priority: "优先级",
      ...
      settingsWebQuotaSync: { enabled: "自动补同步 Web 额度", enabledHelp: "每 30 分钟仅为没有任何额度快照的 Web 账号补同步。关闭后不影响手动同步、生成后的额度刷新和额度恢复探测。" },
      egressProxyProfiles: {
        title: "代理地址库",
        libraryTitle: "代理地址库",
      ...
      docs: {
        ...
        reference: {
          ...
          fieldEditImages: "待编辑图片，使用 image 或 images 传入 URL、Data URL 或已支持的图片引用；grok-imagine-image-2.0（含 Web 路由）最多 14 张。第一张为待编辑原图，其余可作为分段编辑参考图。",
          fieldSelectionRegions: "图层选区，outer.points 为 0–1 的 x,y 交替坐标。多个选区共用顶层 prompt，并映射为上游 multiRegionEdits。不可与 multi_region_edits 同时使用。",
          fieldMultiRegionEdits: "2.0 多段编辑：每组包含 regions、独立 prompt，以及可选的 reference_images。上游走 multiRegionEdits，各组 prompt 用 `; ` 拼接为 message。不可与 selection_regions 同时使用。",
          ...
          noteImageSegmentEdit: "上传图片后的图层选区与多段独立 prompt/参考图均走新会话 multiRegionEdits；分段请求不发送 aspectRatio。",
    ...
    translation: {
      ...
      videoTask: {
        ...
        stages: {
          ...
          acquire_submit_egress: "Preparing video submission egress and Clearance", submit_egress_ready: "Video submission egress ready",
          video_node_rate_limited: "Video submission 429; node cooling for 3 minutes; account quota preserved", select_video_retry_node: "Selecting another video submission proxy",
          video_node_cooldown_failed: "Video submission 429; cooldown could not be saved; stopping retries", video_direct_rate_limited: "Direct video submission 429; account quota preserved",
          video_retry_egress_unavailable: "No alternative submission node; stopping retries", video_rate_limit_retry_limit: "Five nodes attempted; stopping 429 retries",
          statsig_prepare: "Preparing Statsig signature", statsig_cache: "Statsig signature cache hit", statsig_wait: "Obtaining signature or waiting for an in-flight signing request",
          statsig_meta: "Obtaining Statsig page metadata", statsig_meta_index: "Fetching Grok /index signing page", statsig_meta_root: "Falling back to Grok home page",
          statsig_sign_url: "Calling Statsig URL signer", statsig_ready: "Statsig signature preparation complete", statsig_retry: "Refreshing Statsig signature",
          statsig_stale: "Refresh failed; using existing signature", statsig_local_fallback: "Falling back to local Statsig signature", statsig_remote_failed: "Remote Statsig signing failed",
          http_connect: "Acquiring connection / proxy tunnel and TLS", http_send: "Connection ready; sending request", http_wait_headers: "Request sent; waiting for response headers",
          http_first_byte: "First response byte received; reading headers", http_headers: "HTTP response headers received", http_read_body: "Reading HTTP response body",
      ...
      settingsWebQuotaSync: { enabled: "Automatic Web quota catch-up", enabledHelp: "Every 30 minutes, sync only Web accounts that have no quota snapshots. Disabling this does not affect manual sync, post-generation refreshes, or quota recovery probes." },
      ...
      docs: {
        ...
        reference: {
          ...
          fieldEditImages: "Source images supplied as URLs, data URLs, or another supported image reference. grok-imagine-image-2.0, including its Web route, accepts up to 14 images. The first image is the source; additional images can be used as segmented-edit references.",
          fieldSelectionRegions: "Layer selections. outer.points is alternating x,y coordinates in 0–1. Multiple regions share the top-level prompt and map to upstream multiRegionEdits. Do not combine with multi_region_edits.",
          fieldMultiRegionEdits: "2.0 multi-region edits. Each item has regions, its own prompt, and optional reference_images. The upstream uses multiRegionEdits and joins those prompts with `; `. Do not combine with selection_regions.",
          ...
          noteImageSegmentEdit: "Uploaded-image selections and independent prompts/references use multiRegionEdits in a new conversation. Segmented requests omit aspectRatio.",
...
Object.assign(resources["zh-CN"].translation.settings.egress as unknown as Record<string, string>, {
  ...
  scopeWebSubmit: "Grok Web(仅提交)",
  fallbackWebSubmitHelp: "Grok Web 聊天和视频生成提交没有可用出口时使用的回退方式。",
  health: "请求健康",
...
Object.assign(resources.en.translation.settings.egress as unknown as Record<string, string>, {
  ...
  scopeWebSubmit: "Grok Web (submission only)",
  fallbackWebSubmitHelp: "Fallback when no egress is available for Grok Web chat or video generation submission.",
  ...
  operations: "Proxy operations", automation: "Automation", automationHelp: "Periodic node checks, account assignment, and balancing follow their respective switches and intervals.",
  ...
  periodicProbing: "Periodic node checks",
  probeInterval: "Node check interval", probeIntervalHelp: "Disable to stop periodic checks. When enabled, the interval is at least 60 seconds with no configured upper limit. Manual checks and immediate recovery probes remain available.",
...
Object.assign(resources["zh-CN"].translation.settings.egress as unknown as Record<string, string>, {
  automationHelp: "定时节点检测、账号分配和均衡分别按各自的开关和间隔执行。",
  subscriptions: "代理订阅",
  ...
  periodicProbing: "定时检测节点",
  probeIntervalHelp: "关闭后停止定时检测；开启时至少 60 秒，不设上限。手动检测和故障后的即时复测仍可使用。",
