    ...
    translation: {
      ...
      docs: {
        ...
        reference: {
          ...
          fieldSelectionRegions: "图层选区，outer.points 为 0–1 的 x,y 交替坐标。多个选区共用顶层 prompt，映射为上游 selectionRegions。可配合 conversation_id 与 parent_response_id 续写同一会话。不可与 multi_region_edits 同时使用。",
          ...
          fieldImageConversationId: "Imagine 会话 ID。与 parent_response_id 同时提供时，图层编辑会续写 /conversations/{id}/responses。",
          fieldImageParentResponseId: "上一轮 Imagine 响应 ID，作为续写的 parentResponseId。必须与 conversation_id 一起提供。",
          ...
          noteImageSegmentEdit: "图层选区（selection_regions）走 selectionRegions；若同时提供 conversation_id 与 parent_response_id，则续写 /conversations/{id}/responses。多段独立 prompt/参考图走 multi_region_edits → multiRegionEdits。",
    ...
    translation: {
      ...
      docs: {
        ...
        reference: {
          ...
          fieldSelectionRegions: "Layer selections. outer.points is alternating x,y coordinates in 0–1. Multiple regions share the top-level prompt and map to upstream selectionRegions. Pair with conversation_id and parent_response_id to continue the same session. Do not combine with multi_region_edits.",
          ...
          fieldImageConversationId: "Imagine conversation ID. When sent with parent_response_id, layer edits continue /conversations/{id}/responses.",
          fieldImageParentResponseId: "Previous Imagine response ID, sent as parentResponseId. Must be paired with conversation_id.",
          ...
          noteImageSegmentEdit: "Layer selections (selection_regions) map to selectionRegions. When conversation_id and parent_response_id are both set, the gateway continues /conversations/{id}/responses. Independent per-region prompts and references use multi_region_edits → multiRegionEdits.",
