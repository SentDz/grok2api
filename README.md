...
Admins can force cancel queued or running videos from the task status dialog (`POST /api/admin/v1/media/videos/:jobId/cancel`). Cancellation marks the task failed, stops local execution, releases its billing reservation, and prevents stale workers or recovery from restarting it. Workers on other instances check cancellation every second. This does not guarantee that generation already accepted by the upstream stops. Grok Web video submission and result polling fail immediately on HTTP 404, without further polling or account retries.
