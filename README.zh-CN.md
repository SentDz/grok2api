...
开启“视频任务日志”后，任务时间线会细分提交出口准备、Statsig 缓存或并发等待、Grok 页面抓取、URL 签名服务调用、连接建立、请求发送、等待响应头与读取响应内容。每步显示毫秒耗时、实际出口、HTTP 状态和活动请求截止时间。日志不记录签名值、Cookie、代理凭据或请求正文。升级时自动扩展视频任务表约束，允许 `grok_web_submit` 的进度与日志正常落库。

管理员可在“任务状态”弹窗中对排队中或进行中的视频点击“强制取消”（`POST /api/admin/v1/media/videos/:jobId/cancel`）。任务立即标记为失败并停止本地执行，取消计费预留，且不会被旧 worker 更新或恢复重跑；其他实例的 worker 每秒检查取消状态。此操作不保证上游已受理的生成停止。Grok Web 视频提交或结果轮询返回 HTTP 404 时，任务直接失败，不再继续轮询或换号重试。

Web 视频生成提交明确返回 HTTP 429 时，保留账号额度，将实际提交节点冷却 3 分钟，并换账号、换节点重试。冷却独立持久化，代理池模式、已有绑定、连通探测和健康度更新都不能绕过；已经开始的流不会被中断。重试临时切换出口，不修改账号永久绑定；同一任务不再使用已经返回 429 的节点，最多尝试 5 个节点并遵守视频重试上限，没有其他可用节点时立即以限流结束。此策略不适用于上传、下载、轮询或 Statsig 服务的 429。
...
- 订阅和文本/Base64 导入
- 批量探测、筛选、删除、分配与均衡
- 定时节点检测可关闭（`probeIntervalSeconds=0`）；开启时至少 60 秒，不设业务上限。手动检测和故障即时复测仍可使用
- 按作用域配置无回退、直连或固定节点
