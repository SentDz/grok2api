...
// refreshStatsigPair 故意不现算。8 月 19 日和本次启动都证明：用 SVG 公式从首页
// seed/curves 算出的 HEX 会立刻覆盖掉浏览器 digest 抓到的可用 pair，然后 403 code 7。
// 生产只使用 statsig_local.go 冻住的官方 pair，发版后重新抓，不要在这里热更新。
