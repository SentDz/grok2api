...
//
// 2026-09-09 对照 grok.com/imagine 同页 digest 抓包；不要用 statsig_hex.go 现算覆盖。
var (
...
func init() {
	seed, err := decodeStatsigSeed("62/GxIkYy2AWvW+igufpMkTf0VEYX4X8Pf4awTv5dWxpQeEFtcYt+MGmcGCsgGrM")
	if err != nil || len(seed) != 48 {
		return
	}
	statsigSeed = seed
	statsigHEX = "1a1860fae147ae147ae02e147ae147ae1402e147ae147ae140fae147ae147ae00"
}
...
// signStatsigWithMeta 复刻 grok.wodf.de/sign：用请求带来的 metaContent 当 seed，
// 对当前缓存的 curves 算 HEX，再套 70 字节壳。不写全局 pair。
func signStatsigWithMeta(method, path, metaContent string, nowUnix int64) (string, error) {
	seed, err := decodeStatsigSeed(metaContent)
	if err != nil || len(seed) != 48 {
		return "", errors.New("Statsig metaContent 不是 48 字节 seed")
	}
	...
	if strings.TrimSpace(hex) == "" {
		return "", errors.New("Statsig HEX 为空")
	}
	return buildLocalStatsig(seed, hex, method, path, nowUnix)
}
